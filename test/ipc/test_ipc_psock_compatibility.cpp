...
TEST(ipc_write_data_block)
{
    ...
    TEST_ASSERT(0 == rcpr_malloc_allocator_create(&alloc));

    /* create a socket pair for testing. */
    TEST_ASSERT(0 == ipc_socketpair(AF_UNIX, SOCK_STREAM, 0, &lhs, &rhs));

    /* wrap the rhs socket as a psock. */
    TEST_ASSERT(0 == psock_create_from_descriptor(&sock, alloc, rhs));
    ...
    TEST_ASSERT(0 == psock_read_boxed_data(sock, alloc, (void**)&buf, &size));

    /* the size should be correct. */
    TEST_EXPECT(strlen(TEST_STRING) == size);

    /* the data should match. */
    TEST_EXPECT(0 == memcmp(buf, TEST_STRING, size));

    /* clean up. */
    TEST_ASSERT(0 == resource_release(psock_resource_handle(sock)));
    TEST_ASSERT(0 == resource_release(rcpr_allocator_resource_handle(alloc)));
    close(lhs);
}
...
TEST(ipc_write_string_block)
{
    ...
    TEST_ASSERT(0 == psock_read_boxed_string(sock, alloc, &buf, &size));
...
TEST(ipc_write_uint64_block)
{
    ...
    TEST_ASSERT(0 == ipc_write_uint64_block(lhs, EXPECTED_VAL));

    /* read the integer from the psock. */
    TEST_ASSERT(0 == psock_read_boxed_uint64(sock, &val));
...
TEST(ipc_write_int64_block)
{
    ...
    /* write an integer value to the lhs socket. */
    TEST_ASSERT(0 == ipc_write_int64_block(lhs, EXPECTED_VAL));

    /* read the integer from the psock. */
    TEST_ASSERT(0 == psock_read_boxed_int64(sock, &val));
...
TEST(ipc_write_uint8_block)
{
    ...
    /* write an integer value to the lhs socket. */
    TEST_ASSERT(0 == ipc_write_uint8_block(lhs, EXPECTED_VAL));

    /* read the integer from the psock. */
    TEST_ASSERT(0 == psock_read_boxed_uint8(sock, &val));
...
TEST(ipc_write_int8_block)
{
    ...
    /* write an integer value to the lhs socket. */
    TEST_ASSERT(0 == ipc_write_int8_block(lhs, EXPECTED_VAL));

    /* read the integer from the psock. */
    TEST_ASSERT(0 == psock_read_boxed_int8(sock, &val));
...
TEST(ipc_read_data_block)
{
    ...
    TEST_ASSERT(0 == psock_create_from_descriptor(&sock, alloc, lhs));
    ...
    TEST_ASSERT(0 == ipc_read_data_block(rhs, (void**)&buf, &size));
...
TEST(ipc_read_string_block)
{
    ...
    /* write a data value to the lhs socket. */
    TEST_ASSERT(0 == psock_write_boxed_string(sock, TEST_STRING));

    /* read the data from the rhs socket. */
    TEST_ASSERT(0 == ipc_read_string_block(rhs, &buf));
...
TEST(ipc_read_uint64_block)
{
    ...
    /* write an integer value to the lhs socket. */
    TEST_ASSERT(0 == psock_write_boxed_uint64(sock, EXPECTED_VAL));

    /* read the integer from the rhs socket. */
    TEST_ASSERT(0 == ipc_read_uint64_block(rhs, &val));
...
TEST(ipc_read_int64_block)
{
    ...
    /* write an integer value to the lhs socket. */
    TEST_ASSERT(0 == psock_write_boxed_int64(sock, EXPECTED_VAL));

    /* read the integer from the rhs socket. */
    TEST_ASSERT(0 == ipc_read_int64_block(rhs, &val));
...
TEST(ipc_read_uint8_block)
{
    ...
    /* write an integer value to the lhs socket. */
    TEST_ASSERT(0 == psock_write_boxed_uint8(sock, EXPECTED_VAL));

    /* read the integer from the rhs socket. */
    TEST_ASSERT(0 == ipc_read_uint8_block(rhs, &val));
...
TEST(ipc_read_int8_block)
{
    ...
    /* write an integer value to the lhs socket. */
    TEST_ASSERT(0 == psock_write_boxed_int8(sock, EXPECTED_VAL));

    /* read the integer from the rhs socket. */
    TEST_ASSERT(0 == ipc_read_int8_block(rhs, &val));
