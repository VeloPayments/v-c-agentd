...
TEST_SUITE(notificationservice_api_encode_response_test);
...
TEST(argument_nullchecks)
{
    ...
    TEST_EXPECT(
        AGENTD_ERROR_NOTIFICATIONSERVICE_API_BAD_ARGUMENT
            == notificationservice_api_encode_response(
                    nullptr, &size, alloc, method_id, status_code, offset,
    ...
    TEST_EXPECT(
        AGENTD_ERROR_NOTIFICATIONSERVICE_API_BAD_ARGUMENT
            == notificationservice_api_encode_response(
                    &buf, nullptr, alloc, method_id, status_code, offset,
                    payload, payload_size));
    ...
    TEST_EXPECT(
        AGENTD_ERROR_NOTIFICATIONSERVICE_API_BAD_ARGUMENT
            == notificationservice_api_encode_response(
                    &buf, &size, nullptr, method_id, status_code, offset,
                    payload, payload_size));
...
TEST(basics)
{
    ...
    TEST_EXPECT(EXPECTED_STATUS_CODE == ntohl(net_status_code));
