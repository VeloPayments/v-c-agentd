/**
 * \file dataservice/dataservice_api_recvresp_transaction_promote_old.c
 *
 * \brief Read the response from the transaction promote call.
 *
    ...
    ipc_socket_context_t* sock, uint32_t* offset, uint32_t* status)
{
    ...
    retval =
        dataservice_decode_response_transaction_promote(val, size, &dresp);
