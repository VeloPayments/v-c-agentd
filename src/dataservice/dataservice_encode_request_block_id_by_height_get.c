/**
 * \file dataservice/dataservice_encode_request_block_id_by_height_get.c
 *
 * \brief Encode a block id by block height query request.
 *
    ...
    uint64_t height)
{
    ...
    /* copy the block height to the buffer. */
    uint64_t nheight = htonll(height);
    memcpy(breq, &nheight, sizeof(nheight));
    breq += sizeof(nheight);
