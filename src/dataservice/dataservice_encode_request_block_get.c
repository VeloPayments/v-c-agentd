/**
 * \file dataservice/dataservice_encode_request_block_get.c
 *
 * \brief Encode a block by ID get request.
 *
    ...
    const RCPR_SYM(rcpr_uuid)* block_id, bool read_cert)
{
    ...
    /* copy the block id to this buffer. */
    memcpy(breq, block_id, sizeof(*block_id));
    breq += sizeof(*block_id);

    /* copy the read cert flag to this buffer. */
    *breq = (uint8_t)read_cert;
    ++breq;
