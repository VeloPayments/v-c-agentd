/**
 * \file
 * protocolservice/protocolservice_control_api_recvresp_private_key_set.c
 *
 * \brief Receive the response for the private key set control command.
 *
    ...
    int sock, uint32_t* offset, uint32_t* status)
{
    ...
    if (UNAUTH_PROTOCOL_CONTROL_REQ_ID_PRIVATE_KEY_SET != method)
    {
