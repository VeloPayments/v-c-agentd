/**
 * \file protocolservice/protocolservice_api_sendreq_transaction_prev_id_get.c
 *
 * \brief Send the transaction get prev id request.
 *
    ...
    const vccrypt_buffer_t* shared_secret, const uint8_t* txn_id)
{
    ...
    uint32_t net_method_id =
        htonl(UNAUTH_PROTOCOL_REQ_ID_TRANSACTION_ID_GET_PREV);
