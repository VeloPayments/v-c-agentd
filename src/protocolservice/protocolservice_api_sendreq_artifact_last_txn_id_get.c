/**
 * \file protocolservice/protocolservice_api_sendreq_artifact_last_txn_id_get.c
 *
 * \brief Send the artifact get last transaction id request.
 *
    ...
    const vccrypt_buffer_t* shared_secret, const uint8_t* artifact_id)
{
    ...
    uint32_t net_method_id =
        htonl(UNAUTH_PROTOCOL_REQ_ID_ARTIFACT_LAST_TXN_BY_ID_GET);
