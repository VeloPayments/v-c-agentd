/**
 * \file protocolservice/protocolservice_api_recvresp_block_prev_id_get.c
 *
 * \brief Receive the block get prev id response.
 *
    ...
    uint8_t* prev_id)
{
    ...
    *server_iv += 1;
    ...
    if (UNAUTH_PROTOCOL_REQ_ID_BLOCK_ID_GET_PREV != ntohl(val[0]))
    {
    ...
    *offset = ntohl(val[2]);
    ...
    /* verify that the size is large enough for the prev uuid. */
    if (size < 16)
    {
        retval = AGENTD_ERROR_IPC_READ_UNEXPECTED_DATA_SIZE;
        goto cleanup_val;
