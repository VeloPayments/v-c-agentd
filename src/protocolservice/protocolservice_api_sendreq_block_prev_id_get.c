/**
 * \file protocolservice/protocolservice_api_sendreq_block_prev_id_get.c
 *
 * \brief Send the block get prev id request.
 *
    ...
    const vccrypt_buffer_t* shared_secret, const uint8_t* block_id)
{
    ...
    uint32_t net_method_id = htonl(UNAUTH_PROTOCOL_REQ_ID_BLOCK_ID_GET_PREV);
