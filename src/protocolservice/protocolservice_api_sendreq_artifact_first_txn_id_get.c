/**
 * \file protocolservice/protocolservice_api_sendreq_artifact_first_txn_id_get.c
 *
 * \brief Send the artifact get first transaction id request.
 *
    ...
    const vccrypt_buffer_t* shared_secret, const uint8_t* artifact_id)
{
    ...
    uint32_t net_method_id =
        htonl(UNAUTH_PROTOCOL_REQ_ID_ARTIFACT_FIRST_TXN_BY_ID_GET);
    ...
    memcpy(breq + 2 * sizeof(uint32_t), artifact_id, 16);
