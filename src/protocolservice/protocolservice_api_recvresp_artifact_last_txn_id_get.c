/**
 * \file protocolservice/protocolservice_api_recvresp_artifact_last_txn_id_get.c
 *
 * \brief Receive the artifact get last transaction id response.
 *
    ...
    uint8_t* last_txn_id)
{
    ...
    *server_iv += 1;
    ...
    if (UNAUTH_PROTOCOL_REQ_ID_ARTIFACT_LAST_TXN_BY_ID_GET != ntohl(val[0]))
    {
    ...
    *offset = ntohl(val[2]);
    ...
    /* verify that the size is large enough for the id. */
    size_t id_size = 1 * 16;
    if (size < id_size)
    {
        retval = AGENTD_ERROR_IPC_READ_UNEXPECTED_DATA_SIZE;
