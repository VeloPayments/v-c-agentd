    ...
    uint32_t request_offset, const uint8_t* payload, size_t payload_size)
{
    ...
    switch (request_id)
    {
        ...
        case UNAUTH_PROTOCOL_REQ_ID_EXTENDED_API_ENABLE:
            retval =
                protocolservice_protocol_dnd_extended_api_enable(
                    ctx, request_offset, payload, payload_size);
            break;
        ...
        case UNAUTH_PROTOCOL_REQ_ID_EXTENDED_API_SENDRESP:
            retval =
                protocolservice_protocol_dnd_extended_api_sendresp(
                    ctx, request_offset, payload, payload_size);
            break;
