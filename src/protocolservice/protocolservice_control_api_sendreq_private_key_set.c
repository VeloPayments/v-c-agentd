/**
 * \file
 * protocolservice/protocolservice_control_api_sendreq_private_key_set.c
 *
 * \brief Send the private key set request to the protocol service control
 * socket.
 *
    ...
    const vccrypt_buffer_t* entity_sign_privkey)
{
    ...
    uint32_t net_method_id =
        htonl(UNAUTH_PROTOCOL_CONTROL_REQ_ID_PRIVATE_KEY_SET);
