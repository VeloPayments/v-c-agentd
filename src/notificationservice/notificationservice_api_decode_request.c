/**
 * \file notificationservice/notificationservice_api_decode_request.c
 *
 * \brief Decode a request to the notification service connection.
 *
    ...
    const uint8_t** payload, size_t* payload_size)
{
    ...
    if (NULL == buf || NULL == method_id || NULL == offset || NULL == payload
     || NULL == payload_size)
    {
    ...
    size_t computed_header_size =
        sizeof(*method_id) + sizeof(*offset);
